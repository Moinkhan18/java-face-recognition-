...
public class TSCD {
   ...
   private double[][] constructImageMatrix(Face[] faces) {
    System.out.println("✅ Starting constructImageMatrix...");
    System.out.println("Total faces received: " + faces.length);

    if (faces.length == 0 || faces[0] == null || faces[0].getPicture() == null || faces[0].getPicture().getImagePixels() == null) {
        System.out.println("❌ Error: No valid image data found in faces[0].");
        return new double[0][0]; // prevent crash
    }

    int pixelLength = faces[0].getPicture().getImagePixels().length;
    double[][] dpix = new double[faces.length][pixelLength];

    for (int i = 0; i < faces.length; i++) {
        if (faces[i] == null || faces[i].getPicture() == null) {
            System.out.println("⚠ Warning: faces[" + i + "] is null or missing picture.");
            continue;
        }

        double[] pixels = faces[i].getPicture().getImagePixels();
        if (pixels == null || pixels.length != pixelLength) {
            System.out.println("⚠ Warning: Invalid pixel data at faces[" + i + "]");
            continue;
        }
